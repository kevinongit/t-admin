  ...
  getRemoteMovieInfo: function() {
    ...
    return new Promise((resolve, reject) => {
      ...
      .then(function(data) {
        ...
        var theList = getFirstList(data);
        console.log('----theList.movies.length' + theList['movies'].length);
        for (var i=0; i < theList['movies'].length; i++) {
          console.log('++');
          p[i] = bmi.getMovieInfoByName(theList['movies'][i].name);
        }
        ...
        Promise.all(p).then(data => {
            ...
            for(var i=0; i < data.length; i++) {
              const movieItem = getSecondList(theList['movies'][i], data[i]);
              if (movieItem) {
                theMovieList['movies'].push(movieItem);
              }
            }
...
var getSecondList = function(first, data) {
  ...
  console.log("getScondList: movie = '" + movie + "'");
  return movie;
}
